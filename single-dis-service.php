...
$dis_rates            = DIS_CustomFieldsManager::get_field( 'rates', $post->ID );
$dis_get_started      = DIS_CustomFieldsManager::get_field( 'get_started', $post->ID );
$dis_related_doc      = DIS_CustomFieldsManager::get_field( 'related_documents', $post->ID );
$dis_related_services = DIS_CustomFieldsManager::get_field( 'related_services', $post->ID );
$dis_designed_for     = get_the_terms( $post->ID, DIS_USER_STATUS_TAXONOMY );
$dis_offices          = DIS_CustomFieldsManager::get_field( 'office', $post->ID );
$dis_full_offices     = DIS_ContentsManager::get_string_list_from_posts( $dis_offices, false );

// Increment the counter of the visits.
DIS_ContentsManager::increment_visit_counter( 'service_page_counter_enabled', $post->ID );

// Related items.
$dis_related_faqs = DIS_ContentsManager::get_related_faq( $post );
?>
			...
			<?php if ( $dis_related_faqs ) : ?>
				...
				<div class="accordion accordion-left-icon" id="accordionExampleLft">
					<?php foreach ( $dis_related_faqs as $dis_index => $dis_faq ) : ?>
						<?php $dis_item_id = (string) $dis_index . 'l'; ?>
						<div class="accordion-item">
							<h4 class="accordion-header" id="<?php echo esc_attr( 'heading' . $dis_item_id ); ?>">
								<button
									class="accordion-button collapsed"
									type="button"
									data-bs-toggle="collapse"
									data-bs-target="<?php echo esc_attr( '#collapse' . $dis_item_id ); ?>"
									aria-expanded="false"
									aria-controls="<?php echo esc_attr( 'collapse' . $dis_item_id ); ?>"
								>
									<?php echo esc_html( $dis_faq->post_title ); ?>
								</button>
							</h4>
							<div
								id="<?php echo esc_attr( 'collapse' . $dis_item_id ); ?>"
				...
				<?php if ( $dis_related_doc ) : ?>
					...
					<ul class="link-list">
						<?php foreach ( $dis_related_doc as $dis_doc ) : ?>
							<?php
							$dis_attachment_file    = DIS_CustomFieldsManager::get_field( 'file', $dis_doc->ID );
							$dis_attachment_link    = DIS_CustomFieldsManager::get_field( 'link', $dis_doc->ID );
							$dis_documentation_link = $dis_attachment_file ? $dis_attachment_file['url'] : $dis_attachment_link;
							?>
							<li>
								<a target="_blank" href="<?php echo esc_url( $dis_documentation_link ); ?>" rel="noopener noreferrer">
									<?php echo esc_html( $dis_doc->post_title ); ?>
								</a>
							</li>
						<?php endforeach; ?>
					</ul>
