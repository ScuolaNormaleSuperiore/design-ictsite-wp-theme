...
class DIS_ExportManager {
	...
	/**
	 * Admin page slug for the export tool.
	 *
	 * @var string
	 */
	...
	public static function get_export_data_page() {
		?>
		<div class="wrap">
			<h1><?php echo esc_html__( 'Export ICT data', 'design_ict_site' ); ?></h1>
			<p><?php echo esc_html__( 'Export in JSON format', 'design_ict_site' ); ?></p>
			<form method="post">
	 ...
	 * @param string $filename_base File basename.
	 * @return void
	 */
	private static function send_json( $data, $filename_base ) {
		$filename = $filename_base . '-' . gmdate( 'Y-m-d' ) . '.json';
