...
/**
 * Wrapper used for generic search result rendering.
 */
...
class DIS_ContentsManager {
	...
	public static function get_menu_tree_by_items( $menuitems ) {
		...
		foreach ( $menuitems as $item ) {
			...
			} elseif ( array_key_exists( $item->menu_item_parent, $menu_tree ) && null !== $menu_tree[ $item->menu_item_parent ] ) {
	...
	/**
	 * Return service items marked as "Show in Home Page".
	 *
	 * @return array
	 */
	public static function get_service_item_list_for_hp(): array {
		$args  = array(
			'post_type'      => DIS_SERVICE_ITEM_POST_TYPE,
			'posts_per_page' => -1,
			'post_status'    => 'publish',
