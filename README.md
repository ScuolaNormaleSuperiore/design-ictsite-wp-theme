...
## Project status
The project is stable. The current release is [VERSION.md](VERSION.md).
...
## Reuse Catalogue
The project is published in the Developers Italia reuse catalog. [This](https://developers.italia.it/it/software/721253b5-4075-4f9f-b16c-eb3eee57cd36) is the project home page.
...
## Sitemap
The theme provides one HTML sitemap for visitors and, for search engines, two independent sets of XML sitemaps: the theme's own, which is language aware, and the one generated by WordPress. Both describe the same content, and either can be used on its own.

- The HTML sitemap is rendered from a shared sitemap tree and is exposed through the dedicated site map page template.
- The theme XML sitemap is exposed through dedicated endpoints, not through standard WordPress pages.
...
- `wp-sitemap.xml` is the sitemap WordPress generates by itself. It is enabled by default and can be turned off from *WP->ICT Site->Advanced settings->SEO*. Keeping both is harmless, because search engines deduplicate by canonical URL, but only the theme sitemap is guaranteed to follow the Polylang language structure.
...
8. Theme Setup: go to *WP->ICT Site* and set your configuration data. The menu entry is labelled **ICT Site** (*Sito ICT* in Italian) and carries a tools icon.
