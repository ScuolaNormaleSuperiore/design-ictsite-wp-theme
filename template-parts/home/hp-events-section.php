...
if ( $dis_section_enabled ) {
	$dis_items          = DIS_ContentsManager::get_hp_item_list( DIS_EVENT_POST_TYPE );
	$dis_all_items_link = DIS_MultiLangManager::get_archive_link( DIS_EVENT_POST_TYPE );
			...
			<div class="section-content">
				<div class="container">
					<?php if ( $dis_show_title ) : ?>
						<h2 class="pb-4"><?php echo esc_html( dis_ct_data()[ DIS_EVENT_POST_TYPE ]['plural_name'] ); ?></h2>
					<?php else : ?>
						<h2 class="visually-hidden"><?php echo esc_html( dis_ct_data()[ DIS_EVENT_POST_TYPE ]['plural_name'] ); ?></h2>
					<?php endif; ?>
					<div class="row">
						<?php foreach ( $dis_items as $dis_item ) : ?>
							...
							$dis_start_date = DIS_CustomFieldsManager::get_field( 'start_date', $dis_item->ID );

							if ( $dis_start_date ) {
								$dis_timestamp   = strtotime( $dis_start_date );
								$dis_string_date = date_i18n( 'j F Y', $dis_timestamp );
							} else {
								$dis_string_date = '';
							}
								...
								<article class="it-card it-card-image it-card-height-full">
									...
									<footer class="it-card-related it-card-footer">
										<div class="it-card-taxonomy">
											...
											$dis_event_list = null;
											if ( $dis_category ) {
												$dis_event_list = DIS_MultiLangManager::get_page_by_label( EVENTS_PAGE_SLUG );
											}
											?>
										...
										<time class="it-card-date" datetime="<?php echo esc_attr( $dis_string_date ); ?>">
											<?php echo esc_html( $dis_string_date ); ?>
										</time>
									</footer>
								</article>
							</div>
					...
					<div class="text-center pt-5 pb-5">
						<a href="<?php echo esc_url( $dis_all_items_link ); ?>" class="btn btn-secondary">
							<?php echo esc_html__( 'All events', 'design_ict_site' ); ?>
						</a>
					</div>
				</div>
			</div>
